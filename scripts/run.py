...
KNOWN_SOURCES = ["ltds", "vista", "qhts", "cmap", "nhts", "nts", "ktdb"]
...
@click.option(
    ...
    "-s",
    multiple=True,
    type=click.Choice(KNOWN_SOURCES),
    help="Run only these sources (repeatable). Omit to run all.",
)
def main(data_root: str, output: str, source: tuple):
    data_root = Path(data_root).expanduser()
    output = Path(output).expanduser()
    output.mkdir(exist_ok=True, parents=True)
    sources = set(source)  # empty set = run all
    ...
    # LTDS
    # ------------------------------------------------------------------
    if not sources or "ltds" in sources:
    ...
    # VISTA
    # ------------------------------------------------------------------
    if not sources or "vista" in sources:
    ...
    # QHTS
    # ------------------------------------------------------------------
    if not sources or "qhts" in sources:
    ...
    # CMAP
    # ------------------------------------------------------------------
    if not sources or "cmap" in sources:
    ...
    # NHTS
    # ------------------------------------------------------------------
    if not sources or "nhts" in sources:
    ...
    # NTS
    # ------------------------------------------------------------------
    if not sources or "nts" in sources:
    ...
    # KTDB
    # ------------------------------------------------------------------
    if not sources or "ktdb" in sources:
        person_config = load_yaml_config(
